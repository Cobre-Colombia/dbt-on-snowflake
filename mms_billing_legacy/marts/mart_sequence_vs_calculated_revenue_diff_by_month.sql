...
SELECT 
    ...
    COALESCE(CALCULATED_REVENUE, 0) AS CALCULATED_REVENUE_,
    COALESCE(S.NET_TOTAL, 0) AS SEQUENCE_REVENUE_, 
    COALESCE(CALCULATED_REVENUE, 0) - COALESCE(S.NET_TOTAL, 0) AS _DIFFERENCE,
...
FULL OUTER JOIN SEQUENCE_PRODUCT S
   ...
   AND CASE WHEN B.PRODUCT_NAME LIKE '%Discount%' THEN 'discount' ELSE LOWER(B.PRODUCT_NAME) END = LOWER(S.PRODUCT_NAME)
