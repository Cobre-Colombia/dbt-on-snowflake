...
WITH BASE AS (
SELECT SEQUENCE_CUSTOMER_ID, PRODUCT_NAME, MAX(ACCUMULATED_REVENUE) AS CALCULATED_REVENUE
...
SELECT 
    ...
    TO_VARCHAR(CALCULATED_REVENUE - S.NET_TOTAL, '999G999G999G999') || ' COP' AS DIFFERENCE,
    ...
    ROUND(
        ...
        END, 2
    ) AS COP_DIFFERENCE_PERCENT


FROM BASE B
LEFT JOIN SEQUENCE_PRODUCT S
    ON B.SEQUENCE_CUSTOMER_ID = S.CUSTOMER_ID 
   AND LOWER(B.PRODUCT_NAME) = LOWER(S.PRODUCT_NAME)

ORDER BY GROUP_NAME ASC, PRODUCT_NAME ASC, DIFFERENCE