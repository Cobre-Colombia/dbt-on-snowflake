...
WITH BASE AS (
SELECT SEQUENCE_CUSTOMER_ID, GROUP_ID, PRODUCT_NAME, DATE_TRUNC('MONTH', TRANSACTION_MONTH) AS TRANSACTION_MONTH, MAX(ACCUMULATED_REVENUE) AS CALCULATED_REVENUE
...
SELECT LEGAL_NAME, CUSTOMER_ID, PRODUCT_NAME, DATE_TRUNC('MONTH', BILLING_PERIOD_START) AS TRANSACTION_MONTH, SUM(NET_TOTAL) NET_TOTAL
FROM SEQUENCE_
GROUP BY 1, 2, 3, 4
...
ORDER BY GROUP_NAME ASC, CALCULATED_PRODUCT_NAME ASC, BILLING_PERIOD ASC, COP_DIFFERENCE