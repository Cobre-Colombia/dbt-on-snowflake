...
, REMAINING_MINIMUM_AMOUNT AS (
    SELECT DISTINCT SEQUENCE_CUSTOMER_ID, GROUP_ID, 'true up charge' AS PRODUCT_NAME, NULL AS PRICING_TYPE, 'amount' AS TIER_APPLICATION_BASIS, DATE_TRUNC('MONTH', TRANSACTION_MONTH) AS TRANSACTION_MONTH, MAX(REMAINING_MINIMUM_AMOUNT) AS CALCULATED_REVENUE,
...
, BASE AS (
  ...
  UNION ALL
  SELECT * FROM REMAINING_MINIMUM_AMOUNT
)
...
SELECT 
    ...
    ROUND(B.TIER_APPLICATION_VALUE, 2) AS CALCULATED_TIER_APPLICATION_VALUE,
    ROUND(S.TIER_APPLICATION_VALUE, 2) AS SEQUENCE_TIER_APPLICATION_VALUE,
    ...
    COALESCE(ROUND(
            CASE 
                ...
                WHEN CALCULATED_REVENUE_COP = 0 THEN NULL
                ELSE (CALCULATED_REVENUE_COP::FLOAT - SEQUENCE_REVENUE_COP::FLOAT) / CALCULATED_REVENUE_COP::FLOAT * 100
            END, 2
        ), 0)
